            ...
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.isUser ? "justify-end" : "justify-start"}`}>
                <div className={`flex gap-3 max-w-[85%] ${msg.isUser ? "flex-row-reverse" : "flex-row"}`}>
                  ...
                  >
                    {msg.isUser ? <div className="text-sm font-semibold"><User className="w-5 h-5" /></div> : <Bot className="w-5 h-5" />}
                  </div>
                  <div
        ...
        <div className="max-w-3xl mx-auto">
          <div className="relative flex items-end">
            <Textarea
              ...
              className="flex-1 pr-12 py-3 text-base border-gray-200 focus-visible:ring-0 min-h-[60px] max-h-[200px] resize-none"
              rows={1}
            />
            <Button
              onClick={sendMessage}
              size="icon"
              variant="ghost"
              className={`absolute right-2 hover:cursor-pointer z-50 bottom-2 rounded-none ${!input.trim() && "text-gray-300"}`}
              disabled={!input.trim()}
            >
